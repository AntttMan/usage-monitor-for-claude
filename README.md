...
- **Detail popup** (left-click) showing account info, dynamically detected usage bars for all active quota types (Session, Weekly, per-model weekly limits, and any new quotas Anthropic adds) with [configurable field selection](docs/configuration.md#popup-fields), usage credits (spend, monthly limit, and balance), reset countdowns, and a stale-data indicator when values may be outdated
